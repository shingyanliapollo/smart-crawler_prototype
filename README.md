# Smart Crawl - Webコンテンツ取得システム

Firecrawl APIを使用してWebページのコンテンツをMarkdown形式で取得するPythonバッチ処理システムです。Docker環境で動作し、将来的にAPIサーバーの追加も可能な構成になっています。

## 🚀 特徴

- **Webコンテンツ取得**: Firecrawl APIを使用したWebページのスクレイピング
- **Markdown形式**: 取得したコンテンツをMarkdown形式で保存
- **バッチ処理**: URLリストから一括でコンテンツを取得
...
```bash
cp .env.example .env
# .envファイルを編集してFirecrawl APIキーを設定
...
# Firecrawl API設定
FIRECRAWL_API_KEY=your_api_key_here  # Firecrawl APIキー（必須）
```
